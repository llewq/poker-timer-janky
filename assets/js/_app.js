...
function getPlayerList() {
  return playerList = JSON.parse(localStorage.getItem('playerList'));

}

getPlayerList();

let initialPlayerCount;
...
//   nextEliminatedPosition = initialPlayerCount;
// } else {
//   nextEliminatedPosition = JSON.parse(localStorage.getItem('nextEliminatedPosition'));
// }

// function updateNextEliminatedPosition() {
...
function updatePrizePool() {
  let numPayouts = defaultNumPayouts.filter(payout => {
    return payout.entries === initialPlayerCount;
  });

  console.log(initialPlayerCount);

  let payouts = []; // empty array before re-populating

  let placesPaid = numPayouts[0].placesPaid;
  
  let prizeValues = defaultPrizes.filter(prize => {
    return prize.placesPaid === placesPaid;
  });
  
  let payoutPrecentages = prizeValues[0].prizes.sort();
...
function updatePlayer() {
  ...
  playerList[pid].name = this.value;
